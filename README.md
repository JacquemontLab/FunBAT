...
- **run_one_gene_set**: To compute the functional burden association test on one gene set.
- **run_one_gene_set_wih_name**: To compute the functional burden association test on one named gene set.
- **run_multiple_gene_sets**: Parallelized version to compute the functional burden association test on multiple gene sets at once.
- **run_bootstrapped**: Bootstrap version of the functional burden association test for one gene set. Used to assess the contribution of each gene in the gene set to the overall burden association.

More details on how to use each function can be found in the associated jupyter notebook `FunBAT_example.ipynb`.

## Functions

### Function Documentation: `run_one_gene_set`

#### Description
The `run_one_gene_set` function performs a burden analysis for a single gene set. It takes phenotype and variant data, collapses the variants into gene sets, and runs a regression model to assess the association between the gene burden and the phenotype. The function can handle both continuous and binary phenotypes and allows for the inclusion of covariates and interaction terms.

```python
run_one_gene_set(phenotype_table: pd.DataFrame,
...
#### Parameters
 ...
 | `list_covariates` | `list` | List of covariates to include in the regression model in addition to the gene burden. | `None` |
...
### Function Documentation: `run_one_gene_set_wih_name`

#### Description
The `run_one_gene_set_wih_name` function performs a burden analysis for a single gene set with its name. It takes phenotype and variant data, collapses the variants into gene sets, and runs a regression model to assess the association between the gene burden and the phenotype. This function is similar to `run_one_gene_set` but includes the gene set name in the results.

```python
run_one_gene_set_wih_name(phenotype_table: pd.DataFrame,
                             phenotype_column_name: str,
                             variants_table: pd.DataFrame,
                             genes_item: list,
                             keep_all_regression_results: bool = False,
...
### Function Documentation: `run_bootstrapped`

#### Description
The `run_bootstrapped` function performs a bootstrap burden analysis for a list of genes. It creates bootstrap samples of the gene list, performs a burden analysis for each sample, and returns the results. This function is useful for assessing the robustness of the burden analysis results.

```python
run_bootstrapped(phenotype_table: pd.DataFrame,
                   phenotype_column_name: str,
                   variants_table: pd.DataFrame,
                   gene_list: list,
...
For functions analyzing an individual gene set (`run_one_gene_set` and `run_one_gene_set_wih_name`), the gene set should be provided as:
...
For functions analyzing multiple gene sets (run_multiple_gene_sets), the input should be structured as:
